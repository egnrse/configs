...
pause() {
	...
	# repeat, if a wrong option was provided
	echo " No proper option was provided, try again."
	pause $1
	return $?
}
...
if [ $skipMaintenance -eq 0 ]; then
	...
	if [ $(command -v nvim) > /dev/null ]; then
		# for error detection with grep
		nvimError="Error detected while processing command line:"

		# check if nvim starts silently
		nvimOutput="$(nvim --headless +qa 2>&1)"
		if [ -n "${nvimOutput}" ]; then
			echo "WARNING: 'nvim' does not start silently:"
			echo ""
			echo "$nvimOutput"
			echo ""
			echo "$underline"
		...
		if [ -z "$(nvim --headless '+Lazy!' +qa 2>&1 | grep "${nvimError}")" ]; then
		...
		if [ -z "$(nvim --headless '+TSLog' +qa 2>&1 | grep "${nvimError}")" ]; then
			...
			if ! grep -q "${tsuDone}" <<< "$nvimOut"; then
				...
				if [ $? -eq 0 ]; then
					echo ""
					maxTime=$((30*1000)) # timeout 30 sec
					nvimOut=$(nvim --headless "+lua require('nvim-treesitter').update():wait(${maxTime})" +qa 2>&1 | tee /dev/tty)
					echo ""
				fi
				echo "$underline"
			else
